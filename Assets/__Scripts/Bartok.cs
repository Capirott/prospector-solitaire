...
public class Bartok : MonoBehaviour
{
    ...
    internal void CardClicked(CardBartok tCB)
    {
        if (CURRENT_PLAYER.type == PlayerType.human)
        {
            ...
            switch (tCB.state)
            {
                ...
                case CBState.hand:
                    ...
                    {
                        Utils.tr("Bartok:CardClicked()", "Attempted to Play", targetCard.name + "is target");
                    }
                    break;
            }
        }
    ...
    public CardBartok Draw()
    {
        ...
        if (drawPile.Count == 0)
        {
            Utils.tr("Drawing pile");
            int ndx;
            while (discardPile.Count > 0)
            {
                ndx = UnityEngine.Random.Range(0, discardPile.Count);
